...
#--av data/multicam-dataset/Trim_blur/angle-Pen_B_out.avi --cv data/multicam-dataset/Trim_blur/ceiling_out.avi --cl 0
#--av data/multicam-dataset/Trim/angle-Pen_B.avi --cv data/multicam-dataset/Trim/ceiling.avi --cl 0
#--av data/multicam-dataset/test/angle-Pen_B.avi --cv data/multicam-dataset/test/ceiling.avi --cl 0
#--av data/multicam-dataset/Trim_17/angle-Pen_B_17.mp4 --cv data/multicam-dataset/Trim_17/ceiling_Trim_17.mp4 --cl 0
...
class CameraManager(multiproc.context.Process):
    ...
    def __init__(self, angled_camera, ceil_camera, queue, total_pigs, pen_name, ceil_lag):
        ...
        with open(f"data/homography/data_v3.pickle", "rb") as f:
            self.warp_dict['ceiling'] = pickle.load(f)
            print(self.warp_dict['ceiling'])
            self.H = pickle.load(f)
            print(self.H)
            self.warp_dict['angled'] = pickle.load(f)
            print(self.warp_dict['angled'])
    ...
    def ceiling_filter(self, ceil_tracks):
        ...
        for ceil_id in ceil_tracks:
            ...
            if (box_center > 40000 and self.pen_name == "C") or (box_center <= 0 and self.pen_name == "B"):
    ...
    def match_tracks_from_buffer(self):
        ...
        for fid in matching_fids:
            global_position_dict = self.match_tracks(self.ceiling_filter(self.local_buffer['ceiling'].pop(fid, None)), self.local_buffer['angled'].pop(fid, None))
    ...
    def match_tracks(self, ceil_tracks, angled_tracks):
        ...
        for ceil_id in ceil_tracks:
            # GJ
            ceil_to_topceil = self.transform_polygon(self.warp_dict['ceiling'], self.rectangle_to_polygon(ceil_tracks[ceil_id]))  # BB [626.22114081 185.16983677 742.21929358 328.59193591] #xmin, ymin, xmax, ymax
            top_ceil_to_top_angled = self.transform_polygon(self.H, ceil_to_topceil)  # 역함수 대신 그냥 H로 ceil top-> angle top
            top_angled_to_angled[ceil_id] = self.transform_polygon(np.linalg.inv(self.warp_dict['angled']), top_ceil_to_top_angled)
            print(top_angled_to_angled[ceil_id])

        # top_angled_to_angled -> ceil to angle
                ...
                if ceil_id in self.union_find:
                    ...
                    if self.union_find.find(ceil_id) != self.union_find.find(angled_id):
                        ...
                        if min_id not in global_position_dict:
                            self.union_find.connected(ceil_id, min_id)
                            self.union_find.connected(angled_id, min_id)
                        else:
                            self.union_find.connected(ceil_id, max_id)
                            self.union_find.connected(angled_id, max_id)
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--aj', required=False, help="Angled video json")
    parser.add_argument('--cv', required=True, help="Ceiling video stream")
    parser.add_argument('--cj', required=False, help="Ceiling video json")
    ...
    angled_camera = Camera(args.av, aq, track_prefix="a", simulation_file=args.aj)
    ceiling_camera = Camera(args.cv, cq, track_prefix="c", simulation_file=args.cj)

    print("angled_camera start!")
    angled_camera.track()
    print("ceiling_camera start!")
    ceiling_camera.track()

    ## Initialize Camera Manager
    PEN_NAME = "B" if "B" in args.av else "C"
    ...
    w, h = int(f * 640), int(f * 360)
    print(w, h)
    angled_cap = cv2.VideoCapture(args.av)
    ceiling_cap = cv2.VideoCapture(args.cv)
    out = cv2.VideoWriter('multi-tracking_detection_17sss_block.avi', cv2.VideoWriter_fourcc(*'DIVX'), 30, (w, h))
    ...
    while True:
        ...
        # #block
        # cv2.rectangle(angled_frame, (1050, 0), (1920, 100), (255, 255, 255), -1)
        ...
        for pig_id in global_position_dict:
            ...
            # if ceil_box is not None:
            #     ceil_to_topceil = camera_manager.transform_polygon(camera_manager.warp_dict['ceiling'], camera_manager.rectangle_to_polygon(ceil_box))  # BB [626.22114081 185.16983677 742.21929358 328.59193591] #xmin, ymin, xmax, ymax
            #     top_ceil_to_top_angled = camera_manager.transform_polygon(camera_manager.H, ceil_to_topceil)  # 역함수 대신 그냥 H로 ceil top-> angle top
            #     top_angled_to_angled = camera_manager.transform_polygon(np.linalg.inv(camera_manager.warp_dict['angled']), top_ceil_to_top_angled)
            #     cv2.rectangle(angled_frame, (top_angled_to_angled['tl'][0], top_angled_to_angled['tl'][1]), (top_angled_to_angled['br'][0], top_angled_to_angled['br'][1]), (0, 255, 0), 5)

            # if angled_box is not None:
            #     angled_to_top_angled = camera_manager.transform_polygon(camera_manager.warp_dict['angled'], camera_manager.rectangle_to_polygon(angled_box))
            #     top_angled_to_top_ceil = camera_manager.transform_polygon(np.linalg.inv(camera_manager.H), angled_to_top_angled)
            #     topceil_to_ceil = camera_manager.transform_polygon(np.linalg.inv(camera_manager.warp_dict['ceiling']), top_angled_to_top_ceil)
            #     cv2.rectangle(ceil_frame, (topceil_to_ceil['tl'][0], topceil_to_ceil['tl'][1]), (topceil_to_ceil['br'][0], topceil_to_ceil['br'][1]), (0, 255, 0), 5)
